...
program
  ...
  .action(async (projectName: string | undefined, options) => {
    ...
    console.log(chalk.gray('Scaffolding a new Playwright TypeScript project...\n'))
...
async function getProjectConfig(
  ...
  return {
    ...
    installDependencies: options.noInstall ? false : answers.installDependencies ?? true,
    ...answers,
  }
}
...
async function createProject(config: ProjectConfig): Promise<void> {
  ...
  if (includeSampleTests && existsSync(join(templatesPath, 'with-examples'))) {
    ...
    cpSync(join(templatesPath, 'with-examples'), projectPath, { recursive: true })
  }
  ...
  if (useGit) {
    ...
    try {
      ...
      console.log(chalk.yellow('⚠️  Git initialization failed, but project was created successfully'))
    }
  }
  ...
  if (installDependencies) {
    ...
    try {
      ...
      console.log(chalk.yellow('⚠️  Dependency installation failed. Run "npm install" manually.'))
    }
  }
